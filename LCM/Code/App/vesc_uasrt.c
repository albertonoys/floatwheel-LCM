...
void Send_Pack_Data(uint8_t *payload, uint16_t len) {
    ...
    /*
            ...
            起始字节（一个字节） + 数据包长度（一个或两个字节） + 数据包（N个字节） +
       校验（两个字节） + 停止字节（一个字节）
...
void Get_Vesc_Pack_Data(COMM_PACKET_ID id) {
    ...
    if (id == COMM_CHARGE_INFO) {
        ...
        buffer_append_float16(
            command, Charge_Voltage, 10, &ind
        );  // -voltage: 16bit float divided by 10
        buffer_append_float16(
            command, Charge_Current, 10, &ind
...
uint8_t Protocol_Parse(uint8_t *message) {
    ...
    if (crcpayload !=
        (((uint16_t) message[counter + len]) << 8 | ((uint16_t) message[counter + len + 1]))) {
        return 1;  // crc不对
    }
    ...
    case COMM_GET_VALUES:
        ...
        data.avgInputCurrent =
            buffer_get_float32(pdata, 100.0, &ind);  // negative input current implies braking
        ...
        if ((data.rpm > 100) || (data.rpm < -100) || (data.avgInputCurrent > 1) ||
            (data.avgInputCurrent < -1)) {
            data.state = RUNNING;
        } else {
    ...
    case COMM_CUSTOM_APP_DATA:
        ...
        if ((data.state == RUNNING) || (data.state == RUNNING_TILTBACK) ||
