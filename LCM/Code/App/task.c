...
static void lcmConfigReset(void) {
    ...
    if (lcmConfig.statusBarIdleMode < 0 || lcmConfig.statusBarIdleMode >=
    STATUS_BAR_IDLE_MODE_COUNT) { lcmConfig.statusBarIdleMode = DEFAULT_IDLE_MODE;
    }
...
static void WS2812_VESC(void) {
    ...
    if (data.rpm > 250) {
        unsigned int fade =
            1 + (data.rpm - 250) / 25;  // values from 1 to 31 (max rpm allowed is 1000)
        blue = blue / fade;
    }
...
void WS2812_Boot(void) {
    ...
    uint8_t bootAnims[10][3] = {
        // Default (blue...green)
        //{{10,0,30}, {9,3,27}, {8,6,24}, {7,9,21}, {6,12,18}, {5,15,15}, {4,18,12}, {3,21,9},
        //{2,24,6}, {1,27,3}},
        {25, 0, 30},
        {22, 2, 27},
        {18, 4, 24},
        ...
        {0, 18, 6},
        // Rainbow
        //{{30,0,0}, {30,15,0}, {30,30,0}, {15,30,0}, {0,30,0}, {0,30,15}, {0,30,30}, {0,15,30},
        //{0,0,30}, {15,0,30}},
        // Red White Blue
        //{{30,0,0}, {30,30,30}, {0,0,30}, {30,0,0}, {30,30,30}, {0,0,30}, {30,0,0}, {30,30,30},
        //{0,0,30}, {30,0,0}}
    };
    ...
    if (frame >= ANIMATION_TICK_TIME) {
        ...
        for (uint8_t i = 0; i < NUM_LEDS; i++) {
            ...
            uint8_t brightness = (distanceToTail >= 0 && distanceToTail <= TAIL_LENGTH)
                ? WS2812_Measure - distanceToTail * (WS2812_Measure / TAIL_LENGTH)
                : 0;
...
void CheckPowerLevel(float battery_voltage) {
    ...
    // float battcellcurve[10] = {4.054, 4.01, 3.908, 3.827, 3.74, 3.651, 3.571, 3.485, 3.38, 3.0};
    // //P42A {4.07, 4.025, 3.91, 3.834, 3.746, 3.607, 3.49, 3.351, 3.168, 2.81}}; //DG40
...
void Charge_Task(void) {
    ...
    bool isAboveCutoff = false;  // lcmConfig.chargeCutoffVoltage > 0 && Charge_Voltage >
                                 // lcmConfig.chargeCutoffVoltage;
    ...
    if ((data.state < RUNNING_FLYWHEEL) || (ADC1_Val > 2) || (ADC2_Val > 2)) {
        ...
        if (lcmConfig.isSet) {
            ...
            if ((pitch > 75) && (pitch < 105)) {
                // headlights off when the board is upgright (e.g. being carried or leaning against
                // a wall)
                new_brightness = 0;
            }
        }
...
void Usart_Task(void) {
    ...
    static uint8_t commandIndex =
        0;  // Store a rotating index so we can implement relevant frequencies of commands
    uint8_t result;
...
void VESC_State_Task(void) {
    ...
    if (data.state == RUNNING_FLYWHEEL) {
        ...
        if (ADC1_Val < 2.0 && ADC2_Val < 2.0) {
            if ((data.state == 0) || (data.state >= FAULT_ANGLE_PITCH)) {  // disengaged board
                WS2812_Display_Flag = 1;
            }
        } else {
            WS2812_Display_Flag = 2;
